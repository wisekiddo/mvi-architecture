...
import com.wisekiddo.data.mapper.DataMapper
import com.wisekiddo.data.model.DataModel
import com.wisekiddo.data.source.DataSourceFactory
...
class DataRepository @Inject constructor(private val factory: DataSourceFactory,
    ...
    override fun saveDataList(dataList: List<DomainModel>): Completable {
        ...
        dataList.map { dataModel.add(dataMapper.mapToEntity(it)) }
    ...
    override fun getDataList(): Flowable<List<DomainModel>> {
        return factory.retrieveCacheDataStore().isCached()
                .flatMapPublisher {
                    factory.retrieveDataStore(it).getDataList()
                }
                .flatMap {
                    Flowable.just(it.map { dataMapper.mapFromEntity(it) })
                }
                .flatMap {
